...
{
    public const string PolicyName = "OwnerOperations";
...
public sealed class WorkerHeartbeatRegistry
{
    ...
    public void Mark(string workerName) =>
        _heartbeats[workerName] = DateTime.UtcNow;

    public DateTime? Get(string workerName) =>
        _heartbeats.TryGetValue(workerName, out var value)
            ? value
            : null;
}
...
public sealed class ContactService(ClarityDbContext db)
{
    ...
    {
        var normalizedCategory = Categories.Contains(category?.Trim() ?? string.Empty)
            ? category.Trim()
            : "General";
        var normalizedMessage = message?.Trim() ?? string.Empty;

        if (normalizedMessage.Length is < 1 or > 4000)
            throw new InvalidOperationException("Please enter a message between 1 and 4,000 characters.");
        ...
        if (normalizedEmail?.Length > 320)
            throw new InvalidOperationException("The contact email is too long.");
        ...
        var submission = new ContactSubmission
        {
            ...
            SourcePath = string.IsNullOrWhiteSpace(sourcePath)
                ? null
                : sourcePath.Trim()[..Math.Min(sourcePath.Trim().Length, 500)],
...
public sealed class OwnerOperationsService(
    ClarityDbContext db,
    WorkerHeartbeatRegistry heartbeats)
{
    public async Task<OwnerOverview> GetOverviewAsync(CancellationToken cancellationToken = default)
    {
        ...
        return new OwnerOverview(
            ...
            await db.Follows.CountAsync(x => x.Status == FollowStatuses.Active, cancellationToken),
            await db.Follows.CountAsync(
                x => x.Status == FollowStatuses.Error || x.Status == FollowStatuses.NeedsAttention,
                cancellationToken),
            await db.ObservationRuns.CountAsync(
                x => x.Status == ObservationStatuses.Failed && x.StartedAtUtc >= since,
                cancellationToken),
            await db.Notifications.CountAsync(x => x.Status == "Pending", cancellationToken),
            await db.Notifications.CountAsync(x => x.Status == "Failed", cancellationToken),
            await db.FeedbackSubmissions.CountAsync(x => x.CreatedUtc >= since, cancellationToken),
            await db.ContactSubmissions.CountAsync(x => x.CreatedUtc >= since, cancellationToken),
            await dueQuery.CountAsync(cancellationToken),
            oldestDue is null ? null : now - oldestDue.Value,
            heartbeats.Get("observation"),
            heartbeats.Get("expiration"),
            heartbeats.Get("notification"));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var row in runs)
        {
            var owner = await (
                ...
                select new { follow.Id, user.Email, workspace.Name })
                .FirstOrDefaultAsync(cancellationToken);
            ...
            result.Add(new OwnerFailedObservationRow(
                ...
                row.source.AdapterType,
                row.target.PrimaryUri,
                string.IsNullOrWhiteSpace(row.run.ErrorCode) ? "ObservationFailed" : row.run.ErrorCode,
                SanitizeError(row.run.ErrorMessage) ?? "The observation did not complete successfully.",
        ...
        CancellationToken cancellationToken = default)
    {
        var feedback = await db.FeedbackSubmissions
            ...
            .Select(x => new OwnerMessageRow(
                ...
                x.CreatedUtc))
            .ToListAsync(cancellationToken);
        var contact = await db.ContactSubmissions
            .AsNoTracking()
            .OrderByDescending(x => x.CreatedUtc)
            .Take(50)
            .Select(x => new OwnerMessageRow(
